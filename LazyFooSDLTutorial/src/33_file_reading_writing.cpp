...
//SDL_Color highlightColor = { 0xFF, 0xA5, 0, 0xFF };
...
//				// &gData[i] - address of the objects in memory 
//				//			   we're writing
//				// sizeof(Sint32) - number of bytes per object we 
//				//					are writing
//				// 1 - number of objects we are writing
...
//							gDataTextures[currentData].loadFromRenderedText(std::to_string(gData[currentData]), textColor);
//							++currentData;
//							if (currentData > 9) { currentData = 0; }
//							// Rerender current entry input point
//							gDataTextures[currentData].loadFromRenderedText(std::to_string(gData[currentData]), highlightColor);
//							break;
//
...
//				for (int i = 0; i < TOTAL_DATA; ++i) {
//					gDataTextures[i].render((SCREEN_WIDTH - gDataTextures[i].getWidth()) / 2, (i * gDataTextures[i].getHeight()) + gPromptTextTexture.getHeight());
//				}
//
//				//Update screen
